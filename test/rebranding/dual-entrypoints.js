...
assert.match( fourTSL, /from '@tnb1j\/4js\/webgpu'/, '4.js TSL must use the native package path' );
assert.match( threeTSL, /from 'three\/webgpu'/, 'legacy TSL must retain the legacy package path' );
assert.match( threePackageTSL, /from '@tnb1j\/4js\/legacy\/webgpu'/, 'package legacy TSL must use the package legacy path' );
...
const [ packageLegacyWebGPU, packageLegacyTSL ] = await Promise.all( [
	import( '@tnb1j/4js/legacy/webgpu' ),
	import( '@tnb1j/4js/legacy/tsl' )
] );
