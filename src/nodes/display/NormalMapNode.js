	...
	setup( builder ) {
		...
		if ( normalMapType === TangentSpaceNormalMap ) {
			...
			if ( unpackNormalMode === NormalRGPacking ) {
				...
				error( `FOUR.NodeMaterial: Unexpected unpack normal mode: ${ unpackNormalMode }` );
			...
			if ( unpackNormalMode !== NoNormalPacking ) {
				...
				error( `FOUR.NodeMaterial: Normal map type '${ normalMapType }' is not compatible with unpack normal mode '${ unpackNormalMode }'` );
