...
import { add, exp, float, If, Fn, max, texture, uniform, uv, vec2, vec4, luminance, convertToTexture, passTexture, getViewPosition, viewZToPerspectiveDepth, struct, ivec2, mix, property, outputStruct } from '../TSL.js';
 ...
 *
 * @augments TempNode
 * @four_import import { taau } from '@fourjs/core/webgpu';
 */
class TAAUNode extends TempNode {
	...
	constructor( beautyNode, depthNode, velocityNode, camera ) {
		...
		/**
		 * Whether accumulated history must be reseeded.
		 *
		 * @private
		 * @type {boolean}
		 */
	 ...
	 *
	 * @return {TAAUNode} A reference to this node.
	 */
	reset() {
