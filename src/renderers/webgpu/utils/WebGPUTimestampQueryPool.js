...
class WebGPUTimestampQueryPool extends TimestampQueryPool {
	...
	allocateQueriesForContext( uid ) {
		...
		if ( this.currentQueryIndex + 2 > this.maxQueries ) {
			...
			warnOnce( `WebGPUTimestampQueryPool [${ this.type }]: Maximum number of queries exceeded, when using trackTimestamp it is necessary to resolves the queries via renderer.resolveTimestampsAsync( FOUR.TimestampQuery.${ this.type.toUpperCase() } ).` );
			return null;
