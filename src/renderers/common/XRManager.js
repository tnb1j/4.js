...
class XRManager extends EventDispatcher {
	...
	/**
	 * Returns an instance of `FOUR.Group` that represents the transformation
	...
	_validateWebGPUSession() {
		...
		if ( this._session.enabledFeatures.includes( 'webgpu' ) === false ) {
			...
			throw new Error( 'FOUR.XRManager: WebGPU XR sessions require the "webgpu" session feature. Use VRButtonGPU/XRButton with "webgpu" enabled or use a WebGL backend.' );

		}

		if ( renderer.samples > 0 ) {

			warnOnce( 'FOUR.XRManager: WebGPU XR does not support MSAA yet. Disabling MSAA for this XR session.' );
	...
	async _initWebGPUSession( session ) {
		...
		if ( this._useMultiviewIfPossible === true ) {
			...
			warnOnce( 'FOUR.XRManager: WebGPU XR does not support multiview yet. Disabling multiview for this XR session.' );
