...
class Renderer {
	 ...
	 * depends on the selected backend. Either `FOUR.WebGLCoordinateSystem` or
	 * `FOUR.WebGPUCoordinateSystem`.
	 *
	 * @readonly
	 * @type {number}
	 */
	...
	_onDeviceLost( info ) {
		...
		let errorMessage = `FOUR.WebGPURenderer: ${info.api} Device Lost:\n\nMessage: ${info.message}`;
	...
	render( scene, camera ) {
		...
		if ( this._initialized === false ) {
			...
			throw new Error( 'FOUR.Renderer: .render() called before the backend is initialized. Use "await renderer.init();" before rendering.' );
	...
	async getArrayBufferAsync( attribute, target = null, offset = 0, count = - 1 ) {
		...
		if ( offset % 4 !== 0 || ( count > 0 && count % 4 !== 0 ) ) {
			...
			throw new Error( 'FOUR.Renderer: "getArrayBufferAsync()" offset and count must be a multiple of 4.' );
	...
	clear( color = true, depth = true, stencil = true ) {
		...
		if ( this._initialized === false ) {
			...
			throw new Error( 'FOUR.Renderer: .clear() called before the backend is initialized. Use "await renderer.init();" before using this method.' );
	...
	compute( computeNodes, dispatchSize = null ) {
		...
		if ( computeList[ 0 ] === undefined || computeList[ 0 ].isComputeNode !== true ) {
			...
			throw new Error( 'FOUR.Renderer: .compute() expects a ComputeNode.' );
	...
	hasFeature( name ) {
		...
		if ( this._initialized === false ) {
			...
			throw new Error( 'FOUR.Renderer: .hasFeature() called before the backend is initialized. Use "await renderer.init();" before using this method.' );
	...
	initTexture( texture ) {
		...
		if ( this._initialized === false ) {
			...
			throw new Error( 'FOUR.Renderer: .initTexture() called before the backend is initialized. Use "await renderer.init();" before using this method.' );
	...
	initRenderTarget( renderTarget ) {
		...
		if ( this._initialized === false ) {
			...
			throw new Error( 'FOUR.Renderer: .initRenderTarget() called before the backend is initialized. Use "await renderer.init();" before using this method.' );
	...
	_projectObject( object, camera, groupOrder, renderList, clippingContext ) {
		...
		if ( visible ) {
			...
			if ( object.isGroup ) {
				...
				error( 'Renderer: Objects of type FOUR.LineLoop are not supported. Please use FOUR.Line or FOUR.LineSegments.' );
	...
	hasCompatibility( name ) {
		...
		if ( this._initialized === false ) {
			...
			throw new Error( 'FOUR.Renderer: .hasCompatibility() called before the backend is initialized. Use "await renderer.init();" before using this method.' );
