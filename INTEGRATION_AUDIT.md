...
**Baseline:** Three.js r185 source snapshot, rebranded as `@fourjs/core@0.185.1-four.0`
...
| `examples/webgpu_upscaling_taau.html` | The TemporalPipeline example did not map `@fourjs/core/tsl`; the browser failed while loading `Inspector.js` and rendered a black frame. | Added `@fourjs/core/tsl` to the example import map, targeting `../build/4.tsl.js`. | The example and its addon dependency now resolve all package-style specifiers natively in the browser. |
...
| `utils/build/rollup.config.js`<br>`package.json`<br>`build/three.tsl.package.js`<br>`test/rebranding/dual-entrypoints.js` | `@fourjs/core/legacy/tsl` resolved through raw `three/webgpu`, which required an unrelated installed `three` package. | Added a package-specific legacy TSL bundle that imports `@fourjs/core/legacy/webgpu`, mapped the package export to it, and added a binding-identity assertion. | Package consumers receive a self-contained compatibility surface while raw browser/import-map compatibility remains available through `build/three.tsl.js`. |
...
- [x] The real `fourjs-core-0.185.1-four.0.tgz` archive was created and installed into an isolated consumer.
