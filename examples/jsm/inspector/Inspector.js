...
import { setConsoleFunction, REVISION } from '@tnb1j/4js/webgpu';
...
class Inspector extends RendererInspector {
	...
	init() {
		...
		let sign = `FOUR.WebGPURenderer: ${ REVISION } [ "`;
			...
			if ( this.isAvailable ) {
				...
				renderer.init().then( () => {
					...
					if ( renderer.hasFeature( 'timestamp-query' ) !== true ) {
						...
						this.console.addMessage( 'error', 'FOUR.Inspector: GPU Timestamp Queries not available.' );
