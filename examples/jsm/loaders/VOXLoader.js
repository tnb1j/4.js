...
function decodeRotation( byte ) {
	...
	// Convert from VOX coordinate system (Z-up) to 4.js (Y-up)
	// VOX: X-right, Y-forward, Z-up
	// 4.js: X-right, Y-up, Z-backward
	// Transformation: x' = x, y' = z, z' = -y
	//
	// To convert rotation matrix R_vox to R_three:
	// R_three = C * R_vox * C^-1
	// where C converts VOX coords to 4.js coords
...
// Apply VOX transform to a 4.js object
...
// Recursively build 4.js object graph from VOX nodes
...
class VOXLoader extends Loader {
	...
	parse( buffer ) {
		...
		// Build 4.js scene graph from nodes
		let scene = null;
			...
			for ( let vv = 0; vv < dimsV; vv ++ ) {
				...
				for ( let uu = 0; uu < dimsU; ) {
					...
					if ( c !== 0 ) {
						...
						// Convert VOX coords to 4.js coords (Y-up)
						// VOX: X right, Y forward, Z up -> 4.js: X right, Y up, Z back
						const toThree = ( p ) => [
							p[ 0 ] - sx / 2,
							p[ 2 ] - sz / 2,
							- p[ 1 ] + sy / 2
