 ...
 * and building the 4.js scene graph.
 *
...
class USDComposer {
	 ...
	 * @returns {Group} 4.js scene graph
	 */
	...
	/**
	 * Apply USD transforms to a 4.js object.
	 * Handles xformOpOrder with proper matrix composition.
	 * USD uses row-vector convention, 4.js uses column-vector.
	 */
	applyTransform( obj, fields, attrs = {} ) {
		...
		if ( xformOpOrder && xformOpOrder.length > 0 ) {
			...
			// Iterate FORWARD for 4.js column-vector convention
			for ( let i = 0; i < xformOpOrder.length; i ++ ) {
				...
				if ( opName === 'xformOp:transform' ) {
					...
					if ( r ) {
						...
						// 4.js Euler 'ZYX' order produces same result
	...
	_resolveReference( refValue, localVariants = {} ) {
		...
		// If it's already a 4.js Group (legacy support), clone it
	...
	_buildGeomPrimitive( path, spec, typeName ) {
		...
		// USD defaults axis to "Z", 4.js uses Y
	...
	_buildCamera( path ) {
		...
		if ( projection === 'orthographic' ) {
			...
			if ( verticalApertureOffset !== 0 ) {
				...
				// 4.js supports only horizontal film offset directly.
	...
	_applyOpenPBRSurface( material, shaderPath ) {
		...
		if ( thinFilmWeight !== undefined && thinFilmWeight > 0 ) {
			...
			if ( thinFilmThickness !== undefined ) {
				...
				// OpenPBR uses micrometers, 4.js uses nanometers
	...
	_buildSkeleton( path ) {
		...
		for ( let i = 0; i < joints.length; i ++ ) {
			...
			if ( bindTransforms && bindTransforms.length >= ( i + 1 ) * 16 ) {
				...
				// USD matrices are row-major, 4.js is column-major - need to transpose
		...
		for ( const path in this.specsByPath ) {
			...
			if ( rotateXYZSpec?.fields?.timeSamples ) {
				...
				for ( let i = 0; i < times.length; i ++ ) {
					...
					const r = values[ i ];
					// USD rotateXYZ: matrix = Rx * Ry * Rz, use 'ZYX' order in 4.js
					tempEuler.set(
						r[ 0 ] * Math.PI / 180,
						r[ 1 ] * Math.PI / 180,
						r[ 2 ] * Math.PI / 180,
			...
			for ( const prop of properties ) {
				...
				for ( let i = 0; i < times.length; i ++ ) {
					...
					// USD matrices are row-major, 4.js is column-major
					matrix.set(
						m[ 0 ], m[ 4 ], m[ 8 ], m[ 12 ],
						m[ 1 ], m[ 5 ], m[ 9 ], m[ 13 ],
						m[ 2 ], m[ 6 ], m[ 10 ], m[ 14 ],
		...
		if ( rotationsAttr && rotationsAttr.times && rotationsAttr.values ) {
			...
			for ( let jointIdx = 0; jointIdx < joints.length; jointIdx ++ ) {
				...
				for ( let t = 0; t < times.length; t ++ ) {
					...
					// This matches 4.js quaternion order (x,y,z,w)
