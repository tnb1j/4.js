 ...
 *
 * @augments Loader
 * @four_import import { ThreeMFLoader } from '@tnb1j/4js/addons/loaders/3MFLoader.js';
 */
	...
	parse( data ) {
		...
		function loadDocument( data ) {
			...
			try {
				...
				if ( e instanceof ReferenceError ) {
					...
					console.error( 'FOUR.3MFLoader: fflate missing and file is compressed.' );
					return null;
			...
			if ( relsName === undefined ) throw new Error( 'FOUR.ThreeMFLoader: Cannot find relationship file `rels` in 3MF archive.' );
			...
			for ( let i = 0; i < modelPartNames.length; i ++ ) {
				...
				if ( xmlData.documentElement.nodeName.toLowerCase() !== 'model' ) {
					...
					console.error( 'FOUR.3MFLoader: Error loading 3MF - no 3MF document found: ', modelPart );
		...
		function buildMeshes( resourceMap, meshData, objects, modelData, textureData, objectData ) {
			...
			for ( let i = 0, il = keys.length; i < il; i ++ ) {
				...
				switch ( resourceType ) {
					...
					default:
						console.error( 'FOUR.3MFLoader: Unsupported resource type.' );
		...
		function buildObject( objectId, objects, modelData, textureData ) {
			...
			if ( modelData.resources.implicitfunction ) {
				...
				console.warn( 'FOUR.ThreeMFLoader: Implicit Functions are implemented in data-only.', modelData.resources.implicitfunction );
