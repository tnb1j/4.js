 ...
 *
 * @augments Loader
 * @four_import import { PCDLoader } from '@fourjs/core/addons/loaders/PCDLoader.js';
 */
		...
		function decompressLZF( inData, outLength ) {
			...
			do {
				...
				if ( ctrl < ( 1 << 5 ) ) {
					...
					ctrl ++;
					if ( outPtr + ctrl > outLength ) throw new Error( 'FOUR.PCDLoader: Output buffer is not large enough' );
					if ( inPtr + ctrl > inLength ) throw new Error( 'FOUR.PCDLoader: Invalid compressed data' );
					do {
					...
					if ( inPtr >= inLength ) throw new Error( 'FOUR.PCDLoader: Invalid compressed data' );
					if ( len === 7 ) {
					...
					ref -= inData[ inPtr ++ ];
					if ( outPtr + len + 2 > outLength ) throw new Error( 'FOUR.PCDLoader: Output buffer is not large enough' );
					if ( ref < 0 ) throw new Error( 'FOUR.PCDLoader: Invalid compressed data' );
					if ( ref >= outPtr ) throw new Error( 'FOUR.PCDLoader: Invalid compressed data' );
					do {
