 ...
 *
 * @augments Loader
 * @four_import import { MaterialXLoader } from '@fourjs/core/addons/loaders/MaterialXLoader.js';
 */
...
class MaterialXNode {
	...
	getNode( out = null ) {
		...
		if ( this.isConst ) {
			...
			if ( element === 'convert' ) {
				...
				if ( ! nodeElement ) {
					...
					throw new Error( `FOUR.MaterialXLoader: Unexpected node ${ new XMLSerializer().serializeToString( this.nodeXML ) }.` );

				}

				if ( ! nodeElement.nodeFunc ) {

					throw new Error( `FOUR.MaterialXLoader: Unexpected node 2 ${ new XMLSerializer().serializeToString( this.nodeXML ) }.` );
		...
		if ( node === null ) {
			...
			console.warn( `FOUR.MaterialXLoader: Unexpected node ${ new XMLSerializer().serializeToString( this.nodeXML ) }.` );
	...
	setStandardSurfaceToGltfPBR( material ) {
		...
		if ( inputs.thin_film_ior ) {
			...
			// Clamp IOR to valid range for 4.js (1.0 to 2.333)
