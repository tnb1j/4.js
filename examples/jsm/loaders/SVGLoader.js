 ...
 *
 * @augments Loader
 * @four_import import { SVGLoader } from '@fourjs/core/addons/loaders/SVGLoader.js';
 */
class SVGLoader extends Loader {
	...
	parse( text ) {
		...
		function parseNode( node, style ) {
			...
			if ( node.hasAttribute( 'filter' ) ) {
				...
				console.warn( 'FOUR.SVGLoader: Filters are not supported.' );
	...
	static createStrokeMaterial( shapePath ) {
		...
		if ( GRADIENT_URL_RE.test( style.stroke ) ) {
			...
			console.warn( 'FOUR.SVGLoader: Gradient strokes are not supported.' );
