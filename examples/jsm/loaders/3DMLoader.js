 ...
 *
 * @augments Loader
 * @four_import import { Rhino3dmLoader } from '@fourjs/core/addons/loaders/3DMLoader.js';
 */
			...
			if ( texture.image !== null ) {
				...
				switch ( texture.type ) {
					...
					default:
						...
						this.warnings.push( {
							message: `FOUR.3DMLoader: No conversion exists for 3dm ${texture.type}.`,
							type: 'no conversion'
						} );
			...
			if ( this.workerPool.length < this.workerLimit ) {
				...
				worker.onmessage = e => {
					...
					switch ( message.type ) {
						...
						default:
							console.error( 'FOUR.Rhino3dmLoader: Unexpected message, "' + message.type + '"' );
	...
	function extractTextureData( t, tType, d ) {
		...
		if ( image ) {
			...
			self.postMessage( { type: 'warning', id: taskID, data: {
				message: `FOUR.3DMLoader: Image for ${tType} texture not embedded in file.`,
	...
	function extractObjectData( object, doc ) {
		...
		switch ( objectType ) {
			...
			case rhino.ObjectType.Light:
				...
				if ( geometry.lightStyle.name === 'LightStyle_WorldLinear' ) {
					...
					self.postMessage( { type: 'warning', id: taskID, data: {
						message: `FOUR.3DMLoader: No conversion exists for ${objectType.constructor.name} ${geometry.lightStyle.name}`,
						type: 'no conversion',
						guid: _attributes.id
					}
			...
			default:
				...
				self.postMessage( { type: 'warning', id: taskID, data: {
					message: `FOUR.3DMLoader: Conversion not implemented for ${objectType.constructor.name}`,
		...
		if ( geometry ) {
			...
			if ( _attributes.decals().count > 0 ) {
				...
				self.postMessage( { type: 'warning', id: taskID, data: {
					message: 'FOUR.3DMLoader: No conversion exists for the decals associated with this object.',
					type: 'no conversion',
					guid: _attributes.id
				}
			...
			self.postMessage( { type: 'warning', id: taskID, data: {
				message: `FOUR.3DMLoader: ${objectType.constructor.name} has no associated mesh geometry.`,
				type: 'missing mesh',
				guid: _attributes.id
			}
