...
class GLSLDecoder {
	...
	parseFunctionParams( tokens ) {
		...
		for ( let i = 0; i < tokens.length; i ++ ) {
			...
			if ( tokens[ i ] && tokens[ i ].str !== ',' ) throw new Error( 'FOUR.GLSLDecoder: Expected ","' );
	...
	parseStructDefinition() {
		...
		if ( tokens[ 2 ].str !== '{' ) {
			...
			throw new Error( 'FOUR.GLSLDecoder: Expected \'{\' after struct name ' );
		...
		for ( let i = 3; i < tokens.length - 2; i += 3 ) {
			...
			if ( typeToken.type != 'literal' || nameToken.type != 'literal' ) {
				...
				throw new Error( 'FOUR.GLSLDecoder: Invalid struct declaration' );

			}

			if ( tokens[ i + 2 ].str !== ';' ) {

				throw new Error( 'FOUR.GLSLDecoder: Missing \';\' after struct member name' );
		...
		if ( tokens[ tokens.length - 2 ].str !== '}' ) {
			...
			throw new Error( 'FOUR.GLSLDecoder: Missing closing \'}\' for struct ' + structName );
	...
	parseSwitch() {
		...
		if ( this.getToken().str !== '{' ) {
			...
			throw new Error( 'FOUR.GLSLDecoder: Expected \'{\' after switch(...) ' );
