...
function GeometryParametersPanel( editor, object ) {
	...
	const arc = new UINumber( parameters.arc * FOUR.MathUtils.RAD2DEG ).setUnit( '°' ).setStep( 10 ).onChange( update );
	...
	function refreshUI() {
		...
		arc.setValue( parameters.arc * FOUR.MathUtils.RAD2DEG );
	...
	function update() {
		...
		editor.execute( new SetGeometryCommand( editor, object, new FOUR.TorusGeometry(
			radius.getValue(),
			tube.getValue(),
			radialSegments.getValue(),
			tubularSegments.getValue(),
			arc.getValue() * FOUR.MathUtils.DEG2RAD
		) ) );
