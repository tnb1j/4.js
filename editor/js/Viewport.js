...
import { PMREMGenerator } from '@tnb1j/4js/webgpu';
...
function Viewport( editor ) {
	...
	const grid = new FOUR.Group();

	const grid1 = new FOUR.GridHelper( 30, 30 );
	grid1.material.color.setHex( GRID_COLORS_LIGHT[ 0 ] );
	grid1.material.vertexColors = false;
	grid.add( grid1 );

	const grid2 = new FOUR.GridHelper( 30, 6 );
	...
	const selectionBox = new FOUR.Box3Helper( box );
	...
	const onDownPosition = new FOUR.Vector2();
	const onUpPosition = new FOUR.Vector2();
	const onDoubleClickPosition = new FOUR.Vector2();
	...
	signals.rendererCreated.add( function ( newRenderer ) {
		...
		if ( renderer.isWebGLRenderer ) {
			...
			pmremGenerator = new FOUR.PMREMGenerator( renderer );
	...
	signals.sceneBackgroundChanged.add( function ( backgroundType, backgroundColor, backgroundTexture, backgroundEquirectangularTexture, backgroundColorSpace, backgroundBlurriness, backgroundIntensity, backgroundRotation ) {
		...
		switch ( backgroundType ) {
			...
			case 'Color':
				...
				scene.background = new FOUR.Color( backgroundColor );
			...
			case 'Equirectangular':
				...
				if ( backgroundEquirectangularTexture ) {
					...
					backgroundEquirectangularTexture.mapping = FOUR.EquirectangularReflectionMapping;
					...
					scene.backgroundRotation.y = backgroundRotation * FOUR.MathUtils.DEG2RAD;
		...
		switch ( environmentType ) {
			...
			case 'Equirectangular':
				...
				if ( environmentEquirectangularTexture ) {
					...
					scene.environment.mapping = FOUR.EquirectangularReflectionMapping;
	...
	signals.sceneFogChanged.add( function ( fogType, fogColor, fogNear, fogFar, fogDensity ) {
		...
		switch ( fogType ) {
			...
			case 'None':
				...
				break;
			case 'Fog':
				scene.fog = new FOUR.Fog( fogColor, fogNear, fogFar );
				break;
			case 'FogExp2':
				scene.fog = new FOUR.FogExp2( fogColor, fogDensity );
				break;
	...
	signals.viewportShadingChanged.add( function () {
		...
		switch ( viewportShading ) {
			...
			case 'normals':
				scene.overrideMaterial = new FOUR.MeshNormalMaterial();
				break;

			case 'wireframe':
				scene.overrideMaterial = new FOUR.MeshBasicMaterial( { color: 0x000000, wireframe: true } );
				break;
	...
	const timer = new FOUR.Timer(); // only used for animations
