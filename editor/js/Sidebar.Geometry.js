...
function SidebarGeometry( editor ) {
		...
		switch ( action ) {
			...
			case 'Flatten':
				...
				let cmds = [ new SetGeometryCommand( editor, object, newGeometry ),
					new SetPositionCommand( editor, object, new FOUR.Vector3( 0, 0, 0 ) ),
					new SetRotationCommand( editor, object, new FOUR.Euler( 0, 0, 0 ) ),
					new SetScaleCommand( editor, object, new FOUR.Vector3( 1, 1, 1 ) ) ];
	...
	*/
	...
	const geometryUUIDRenew = new UIButton( strings.getKey( 'sidebar/geometry/new' ) ).setMarginLeft( '7px' ).onClick( function () {
		...
		geometryUUID.setValue( FOUR.MathUtils.generateUUID() );
