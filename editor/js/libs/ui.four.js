		...
		async function loadFile( file ) {
			...
			if ( cache.has( hash ) ) {
				...
				reader.addEventListener( 'load', async function ( event ) {
					...
					const { HDRLoader } = await import( '@tnb1j/4js/addons/loaders/HDRLoader.js' );
				...
				reader.addEventListener( 'load', async function ( event ) {
					...
					const { TGALoader } = await import( '@tnb1j/4js/addons/loaders/TGALoader.js' );
				...
				reader.addEventListener( 'load', async function ( event ) {
					...
					const { EXRLoader } = await import( '@tnb1j/4js/addons/loaders/EXRLoader.js' );
				...
				reader.addEventListener( 'load', function ( event ) {
					...
					image.addEventListener( 'load', function () {
						...
						const texture = new FOUR.Texture( this );
						texture.sourceFile = file.name;
						texture.needsUpdate = true;
...
class UIPoints2 extends UIPoints {
	...
	getValue() {
		...
		for ( let i = 0; i < this.pointsUI.length; i ++ ) {
			...
			points.push( new FOUR.Vector2( pointUI.x.getValue(), pointUI.y.getValue() ) );
			++ count;
...
class UIPoints3 extends UIPoints {
	...
	getValue() {
		...
		for ( let i = 0; i < this.pointsUI.length; i ++ ) {
			...
			points.push( new FOUR.Vector3( pointUI.x.getValue(), pointUI.y.getValue(), pointUI.z.getValue() ) );
			++ count;
			pointUI.lbl.setValue( count );
...
function renderToCanvas( texture ) {
	...
	if ( fsQuad === undefined ) {
		...
		fsQuad = new FullScreenQuad( new FOUR.MeshBasicMaterial() );
