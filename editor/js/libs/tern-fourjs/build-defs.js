// Builds Tern definitions for the 4.js API by scanning the JSDoc comments
// in a 4.js build file (e.g. build/4.core.js).
//
...
// TernServer:  { "!name": "threejs", "FOUR": { Box3: { prototype: { ... } } } }
...
function mapType( raw, classes ) {
	...
	if ( classes.has( type ) ) return '+FOUR.' + type;
...
export default function buildDefs( source ) {
	...
	// pass 1: collect class names (so types can resolve to +FOUR.X)
	...
	for ( let i = 0; i < lines.length; i ++ ) {
		...
		if ( cm ) {
			...
			curName = cm[ 1 ];
			cur = FOUR[ curName ] || ( FOUR[ curName ] = {} );
			// Declaring the class with a function `!type` makes Tern treat it as a
			// constructor, so `new FOUR.X()` resolves to X.prototype. The signature
			// is refined once the constructor's JSDoc is read.
			cur[ '!type' ] = 'fn()';
			cur.prototype = cur.prototype || {};
			if ( cm[ 2 ] && classes.has( cm[ 2 ] ) ) cur.prototype[ '!proto' ] = 'FOUR.' + cm[ 2 ] + '.prototype';
