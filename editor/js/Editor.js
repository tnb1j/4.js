...
var _DEFAULT_CAMERA = new FOUR.PerspectiveCamera( 50, 1, 0.001, 1e10 );
_DEFAULT_CAMERA.name = 'Camera';
_DEFAULT_CAMERA.position.set( 0, 5, 10 );
_DEFAULT_CAMERA.lookAt( new FOUR.Vector3() );
...
function Editor() {
	...
	this.sceneHelpers = new FOUR.Scene();
	this.sceneHelpers.add( new FOUR.HemisphereLight( 0xffffff, 0x888888, 2 ) );
	...
	this.mixer = new FOUR.AnimationMixer( this.scene );
	...
	this.viewportColor = new FOUR.Color();
...
Editor.prototype = {
	...
	addHelper: function () {
		...
		var geometry = new FOUR.SphereGeometry( 2, 4, 2 );
		var material = new FOUR.MeshBasicMaterial( { color: 0xff0000, visible: false } );
		...
		return function ( object, helper ) {
			...
			if ( helper === undefined ) {
				...
				if ( object.isCamera ) {
					...
					helper = new FOUR.CameraHelper( object );

				} else if ( object.isPointLight ) {

					helper = new FOUR.PointLightHelper( object, 1 );

					helper.matrix = new FOUR.Matrix4();
					...
					helper = new FOUR.DirectionalLightHelper( object, 1 );

				} else if ( object.isSpotLight ) {

					helper = new FOUR.SpotLightHelper( object );

				} else if ( object.isHemisphereLight ) {

					helper = new FOUR.HemisphereLightHelper( object, 1 );

				} else if ( object.isSkinnedMesh ) {

					helper = new FOUR.SkeletonHelper( object.skeleton.bones[ 0 ] );

				} else if ( object.isBone === true && object.parent && object.parent.isBone !== true ) {

					helper = new FOUR.SkeletonHelper( object );
				...
				const picker = new FOUR.Mesh( geometry, material );
				picker.name = 'picker';
	...
	setCameraType: function ( type ) {
		...
		const center = this.controls ? this.controls.center : new FOUR.Vector3();
		...
		if ( type === 'orthographic' ) {
			...
			newCamera = new FOUR.OrthographicCamera( - halfSize, halfSize, halfSize, - halfSize, 0, 10000 );
	...
	fromJSON: async function ( json ) {
		...
		var loader = new FOUR.ObjectLoader();
