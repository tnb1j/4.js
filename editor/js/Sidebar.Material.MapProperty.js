...
import { UITexture } from './libs/ui.four.js';
...
function SidebarMaterialMapProperty( editor, property, name ) {
	...
	function onChange() {
		...
		if ( material[ property ] !== newMap ) {
			...
			if ( newMap !== null ) {
				...
				if ( property === 'envMap' ) newMap.mapping = FOUR.EquirectangularReflectionMapping;
	...
	function onMapChange( texture ) {
		...
		if ( texture !== null ) {
			...
			if ( colorMaps.includes( property ) && texture.isDataTexture !== true && texture.colorSpace !== FOUR.SRGBColorSpace ) {
