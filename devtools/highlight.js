// This script handles highlighting of 4.js objects in the 3D scene
...
( function () {
	...
	function highlight( uuid ) {
		...
		const devTools = globalThis.__FOUR_DEVTOOLS__ || globalThis.__THREE_DEVTOOLS__;
		const object = devTools.utils.findObjectInScenes( uuid );
		if ( ! object ) {
		...
		if ( object.type.includes( 'Helper' ) || object.name === '__FOUR_DEVTOOLS_HIGHLIGHT__' || ! object.geometry ) {
		...
		highlightObject.name = '__FOUR_DEVTOOLS_HIGHLIGHT__';
	...
	devTools.addEventListener( 'highlight-object', ( event ) => {

		highlight( event.detail.uuid );

	} );

	devTools.addEventListener( 'unhighlight-object', () => {
