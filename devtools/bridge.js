...
/**
 * This script injected by the installed 4.js developer
 * tools extension.
 */
...
( function () {
	...
	// Only initialize if neither the native nor compatibility bridge exists.
	if ( ! window.__FOUR_DEVTOOLS__ && ! window.__THREE_DEVTOOLS__ ) {
		...
		// Expose one event target under both identities so the extension can
		// inspect native 4.js and compatibility 4.js bundles.
		const devTools = new DevToolsEventTarget();

		for ( const property of [ '__FOUR_DEVTOOLS__', '__THREE_DEVTOOLS__' ] ) {
		...
		function traverseObjectTree( rootObject, callback, skipDuplicates = false ) {
			...
			function traverse( object ) {
				...
				if ( object.name === '__FOUR_DEVTOOLS_HIGHLIGHT__' ) return;
		...
		function checkBridgeAvailability() {
			...
			const devToolsValue = window.__FOUR_DEVTOOLS__ || window.__THREE_DEVTOOLS__;
		...
		// Check if FOUR is in the global scope (Old versions)
		window.addEventListener( 'load', () => {

			if ( window.FOUR && window.FOUR.REVISION ) {

				dispatchEvent( EVENT_REGISTER, { revision: window.FOUR.REVISION } );
		...
		function sendState() {
			...
			// Send current scenes. 4.js scenes have no dispose() method, so we
