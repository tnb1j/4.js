...

{
	...
	function fogExample( scene, fog, update ) {
		...
		const material = new FOUR.MeshPhongMaterial( { color: 'hsl(130,50%,50%)' } );
		return {
	...
	function houseScene( props, fogInHouse ) {
		...
		scene.background = new FOUR.Color( '#FFF' );
		camera.far = 200;
		...
		loader.load( '/manual/examples/resources/models/simple_house_scene/scene.gltf', ( gltf ) => {
			...
			const hackGeometry = new FOUR.CircleGeometry( 0.5, 32 );
			const box = new FOUR.Box3();
			const size = new FOUR.Vector3();
			const center = new FOUR.Vector3();
			...
			gltf.scene.traverse( ( node ) => {
				...
				if ( material ) {
					...
					if ( node.name === 'mesh_11' || node.name === 'mesh_6' ) {
						...
						const hackMesh = new FOUR.Mesh( hackGeometry, node.material );
						scene.add( hackMesh );
		...
		const near = 1.5;
		const far = 5;
		scene.fog = new FOUR.Fog( color, near, far );
	...
	threejsLessonUtils.addDiagrams( {
		fog: {
			create( props ) {
				...
				const near = 12;
				const far = 18;
				const fog = new FOUR.Fog( color, near, far );
		...
		fogExp2: {
			create( props ) {
				...
				const color = 0xFFFFFF;
				const density = 0.1;
				const fog = new FOUR.FogExp2( color, density );
				return fogExample( scene, fog, createLightDarkFogUpdater( fog ) );
		...
		fogBlueBackgroundRed: {
			create( props ) {
				...
				const { scene } = props;
				scene.background = new FOUR.Color( '#F00' );
				const color = '#00F';
				const near = 12;
				const far = 18;
				return fogExample( scene, new FOUR.Fog( color, near, far ) );
		...
		fogBlueBackgroundBlue: {
			create( props ) {
				...
				const { scene } = props;
				scene.background = new FOUR.Color( '#00F' );
				const color = '#00F';
				const near = 12;
				const far = 18;
				return fogExample( scene, new FOUR.Fog( color, near, far ) );
