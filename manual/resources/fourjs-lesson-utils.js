...
export const threejsLessonUtils = {
	...
	init( options = { threejsOptions: {} } ) {
		...
		const clearColor = new FOUR.Color( '#000' );
	...
	addDiagram( elem, info ) {
		...
		const near = 0.1;
		const far = 50;
		let camera = new FOUR.PerspectiveCamera( targetFOVDeg, aspect, near, far );
		...
		promise.then( ( result ) => {
			...
			const info = result instanceof FOUR.Object3D ? {
				obj3D: result,
			} : result;
			if ( info.obj3D ) {
			...
			if ( settings.lights !== false ) {
				...
				camera.add( new FOUR.HemisphereLight( 0xaaaaaa, 0x444444, .5 ) );
		...
		const render = ( renderer, time ) => {
          ...
          rect.right < 0 || rect.left > renderer.domElement.clientWidth ) {
			...
			const fovDeg = aspect >= 1
				? targetFOVDeg
				: FOUR.MathUtils.radToDeg( 2 * Math.atan( Math.tan( FOUR.MathUtils.degToRad( targetFOVDeg ) * .5 ) / aspect ) );
