	...
	threejsLessonUtils.addDiagrams( {
		mergedCubes: {
			create() {
				...
				for ( let y = 0; y < height; ++ y ) {
					...
					for ( let z = 0; z < depth; ++ z ) {
						...
						for ( let x = 0; x < width; ++ x ) {
							...
							geometry.applyMatrix4( ( new FOUR.Matrix4() ).makeTranslation( x, y, z ) );
				...
				const mesh = new FOUR.Mesh( mergedGeometry, material );
				mesh.position.set(
					0.5 - width / 2,
					0.5 - height / 2,
					0.5 - depth / 2 );
		...
		culledCubes: {
			create() {
				...
				const geometry = new FOUR.BoxGeometry( 3, 2, 2, 3, 2, 2 );
				const material = new FOUR.MeshBasicMaterial( {
					color: colors.wire,
					wireframe: true,
				} );
