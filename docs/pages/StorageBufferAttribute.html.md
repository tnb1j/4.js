...
This special type of buffer attribute is intended for compute shaders. In earlier 4.js versions it was only possible to update attribute data on the CPU via JavaScript and then upload the data to the GPU. With the new material system and renderer it is now possible to use compute shaders to compute the data for an attribute more efficiently on the GPU.
...
### new StorageBufferAttribute( count : number | TypedArray, itemSize : number, typeClass : TypedArray.constructor )
...
### .isStorageBufferAttribute : boolean (readonly)
...
[src/renderers/common/StorageBufferAttribute.js](../../src/renderers/common/StorageBufferAttribute.js)